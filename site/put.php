...
case "adduser":
  ...
  if($op=="adicionar"){  
    ...
    $datafim=$json_obj->{'jsondata'}->{'datafim'};
    $nome=$json_obj->{'jsondata'}->{'nome'};
    $descricao=urldecode($json_obj->{'jsondata'}->{'descricao'});
    $sql="INSERT INTO eventos(idadmin,idestilo,idartista,idcategoria,idlocal,datahorario,datafim,nome,descricao) VALUES($idadmin,$idestilo,$idartista,$idcategoria,$idlocal,'$datahorario','$datafim','$nome','$descricao')";  
    try {
      $qr=$pdo->query($sql);
      if($qr) {     
